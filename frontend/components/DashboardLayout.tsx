...
export const menuItems = [
  ...
  { href: "/admin/ecommerce", label: "Operations (E-commerce)", icon: ShoppingCart },
...
export default function DashboardLayout({ 
  ...
  if (role === "sales") {
    ...
    actMenuItems = [
      ...
      { href: "/shipper/deliveries", label: "My Deliveries", icon: Box },
    ];
  }
    ...
    <div className="flex h-screen bg-dark-bg text-slate-300 font-sans overflow-hidden">
      ...
      <aside className={`${isSidebarOpen ? "w-64" : "w-20"} transition-all duration-300 bg-dark-panel border-r border-dark-border flex flex-col z-20`}>
        ...
        <div className="p-4 border-t border-dark-border">
          <Link href="/login" className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-slate-400 hover:text-neon-red hover:bg-neon-red/10 transition-colors ${!isSidebarOpen && "justify-center"}`}>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="h-16 flex items-center justify-between px-6 bg-dark-panel/80 backdrop-blur-md border-b border-dark-border flex-shrink-0 z-10">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-neon-green/10 border border-neon-green/30 text-neon-green text-xs font-semibold shadow-[0_0_10px_rgba(0,255,65,0.1)]">
              <span className="w-2 h-2 rounded-full bg-neon-green animate-pulse"></span>
              Gateway: Active (Zero Trust)
            </div>
        ...
        {showAlertModal && (
          <div className="absolute top-20 right-6 w-96 bg-dark-panel border border-neon-red/50 rounded-xl shadow-2xl p-4 z-50 animate-in slide-in-from-top-4 fade-in">
            ...
            <div className="flex gap-2">
              <button className="flex-1 py-1.5 text-xs font-semibold bg-dark-bg border border-dark-border text-slate-300 hover:text-white rounded hover:border-neon-blue transition-colors">Investigate Logs</button>
